...
/*
  FIVE-PRODUCT COMPARISON TABLE

  Purpose:
    Produce an advisor-ready P1-P5 structural/evidence comparison
    from the K=5 topology and unified model.
...
const OUTDIR = path.join(ROOT, "results", "five-product-comparison");
...
if (Number(topology.ecosystem.K) !== 5) {
  throw new Error(`Expected K=5 topology, got K=${topology.ecosystem.K}`);
}

if (Number(unified.K) !== 5) {
  throw new Error(`Expected K=5 unified model, got K=${unified.K}`);
}
...
const components = [
  { id: "N1", component: "depd@2.0.0", kind: "NODE" },
  { id: "E1", component: "debug@2.6.9 -> ms@2.0.0", kind: "EDGE" },
  { id: "E2", component: "on-finished@2.4.1 -> ee-first@1.1.1", kind: "EDGE" },
];
...
for (const p of ["P1", "P2", "P3", "P4", "P5"]) {
  ...
  for (const c of components) {
    ...
    if (!modelComp) throw new Error(`Missing component ${c.id} in unified model`);
...
const evidenceRows = [
  {
    productId: "P1",
    product: "Express",
    N1: "Studied in depd activation/perturbation experiments",
  ...
  },
  {
    ...
    N1: "Studied in depd activation/perturbation experiments",
    E1: "Studied in debug->ms activation/propagation experiments",
...
fs.writeFileSync(
  path.join(OUTDIR, "p1-p5-structural-comparison.csv"),
  structuralCsv,
  "utf8"
);
...
fs.writeFileSync(
  path.join(OUTDIR, "p1-p5-component-membership.csv"),
  membershipCsv,
  "utf8"
);
...
fs.writeFileSync(
  path.join(OUTDIR, "p1-p5-evidence-comparison.csv"),
  evidenceCsv,
  "utf8"
);
...
report.push("FIVE-PRODUCT COMPARISON");
report.push("=======================");
report.push("");
...
report.push("");
report.push("K=4 -> K=5 CHANGE SUMMARY");
report.push("-------------------------");
report.push(
  "N1 depd@2.0.0: structural reach decreased because P5 does not contain the exact node."
);
report.push(
  "E1 debug@2.6.9 -> ms@2.0.0: structural reach increased because P5 contains the exact edge and also reproduced the pre-specified runtime mechanism."
);
report.push(
  "E2 on-finished@2.4.1 -> ee-first@1.1.1: structural reach decreased because P5 does not contain the exact edge."
);
...
report.push("--------------");
report.push(
  "The five products differ in topology, exact component membership, and experimentally observed runtime behavior."
);
report.push(
  "P4 Connect and P5 Compression provide successive cross-product validation environments for E1."
);
report.push(
  "The repeated E1 validation supports mechanism generality for the reused exact edge across multiple parent-product environments."
);
report.push(
...
fs.writeFileSync(
  path.join(OUTDIR, "five-product-comparison-report.txt"),
  report.join("\n"),
  "utf8"
);
...
console.log("  results\\five-product-comparison\\five-product-comparison-report.txt");
console.log("  results\\five-product-comparison\\p1-p5-structural-comparison.csv");
console.log("  results\\five-product-comparison\\p1-p5-component-membership.csv");
console.log("  results\\five-product-comparison\\p1-p5-evidence-comparison.csv");
