...
/*
  FOUR-PRODUCT ECOSYSTEM TOPOLOGY REBUILD — CORRECTED

  Correction:
  1) DAG/cycle analysis is performed on PHYSICAL installed package instances
     (package-lock paths), not on collapsed name@version identities.
  2) Cross-product sharing still uses exact identity name@version.
  3) Root -> direct-dependency edges are reported separately so individual
     product counts can be compared consistently with earlier work.
*/
...
const OUTDIR = path.join(ROOT, "results", "ecosystem-topology-4product-corrected");
...
function dependencyCandidate(baseKey, depName) {
  return baseKey
    ? `${normalizeKey(baseKey)}/node_modules/${depName}`
...
function graphStats(nodes, edges) {
  ...
  if (!isDAG) {
    return { isDAG: false, maxDepth: null };
  }
  ...
  return {
    isDAG: true,
    maxDepth: Math.max(0, ...dist.values()),
  };
}
...
function readProduct(product) {
  ...
  if (!fs.existsSync(lockFile)) {
    throw new Error(`Missing package-lock.json: ${lockFile}`);
  }

  const lock = JSON.parse(fs.readFileSync(lockFile, "utf8"));

  if (!lock.packages || typeof lock.packages !== "object") {
    throw new Error(`No packages map in ${lockFile}`);
  }
  ...
  const physicalNodesWithRoot = [rootNode, ...physical.keys()];
  const physicalEdgesWithRoot = [
    ...rootEdges,
    ...physicalEdges.map(e => ({ source: e.source, target: e.target })),
  ];

  const physicalGraph = graphStats(
    physicalNodesWithRoot,
    physicalEdgesWithRoot
  );
...
function buildEcosystem(products) {
  ...
  for (const p of products) {
    for (const node of p.exactNodes) {
      ...
      if (!versionsByNameAcrossProducts.has(name)) {
        versionsByNameAcrossProducts.set(name, new Map());
      }
  ...
  return {
    ...
    sumIndividualPackageNodeInstances: products.reduce(
      (s, p) => s + p.packageNodeInstances, 0
    ),

    sumIndividualExactIdentityCounts: products.reduce(
      (s, p) => s + p.exactPackageVersionIdentities, 0
    ),
    ...
    sumIndividualNonRootPhysicalEdges: products.reduce(
      (s, p) => s + p.nonRootPhysicalDependencyEdges, 0
    ),

    sumIndividualRootDirectEdges: products.reduce(
      (s, p) => s + p.rootDirectEdges, 0
    ),

    sumIndividualTotalPhysicalEdgesIncludingRoot: products.reduce(
      (s, p) => s + p.totalPhysicalEdgesIncludingRoot, 0
    ),
...
function main() {
  ...
  const output = {
    analysis: "Corrected four-product ecosystem topology rebuild",
    correction:
      "Physical install-path instances are used for DAG/cycle analysis; exact name@version identities are used for cross-product sharing.",
  ...
  fs.writeFileSync(
    path.join(OUTDIR, "ecosystem-topology-4product-corrected.json"),
    JSON.stringify(output, null, 2),
    "utf8"
  );

  writeCsv(
    path.join(OUTDIR, "shared-nodes-4product-corrected.csv"),
    ["node", "productCount", "products", "rho"],
    ecosystem.sharedNodes
  );

  writeCsv(
    path.join(OUTDIR, "shared-edges-4product-corrected.csv"),
  ...
  report.push("CORRECTED FOUR-PRODUCT ECOSYSTEM TOPOLOGY REBUILD");
  report.push("=================================================");
  report.push("");
  report.push("CORRECTION");
  report.push("----------");
  report.push(
    "DAG/cycle analysis uses physical installed package instances (package-lock paths)."
  );
  report.push(
    "Exact name@version identities are used only for cross-product equivalence/sharing."
  );
  report.push(
    "This avoids artificial cycles created by collapsing two physically separate installs into one exact identity."
  );
  report.push("");
  ...
  for (const p of products) {
    ...
    report.push(
      `  unique exact name@version identities within product=${p.exactPackageVersionIdentities}`
    );
    report.push(
      `  non-root physical dependency edges=${p.nonRootPhysicalDependencyEdges}`
    );
    report.push(
      `  unique exact non-root dependency edges=${p.uniqueExactNonRootDependencyEdges}`
    );
    ...
    report.push(
      `  physical DAG=${p.physicalDAG ? "YES" : "NO"} maxDepthIncludingRoot=${p.physicalMaxDepthIncludingRoot ?? "NA"}`
    );
    report.push(
      `  package names with multiple installed versions=${p.duplicatePackageNameCount}`
    );
    report.push(
      `  unresolved dependency edges=${p.unresolvedDependencies.length}`
    );
  }
  ...
  report.push(
    `Sum individual package node instances=${ecosystem.sumIndividualPackageNodeInstances}`
  );
  report.push(
    `Sum individual exact-identity counts=${ecosystem.sumIndividualExactIdentityCounts}`
  );
  report.push(
    `Unique exact nodes across ecosystem=${ecosystem.uniqueExactNodesAcrossEcosystem}`
  );
  report.push(
    `Exact nodes shared by >=2 products=${ecosystem.exactNodesSharedGE2}`
  );
  report.push(
    `Exact nodes shared by all ${ecosystem.K} products=${ecosystem.exactNodesSharedAllK}`
  );
  report.push(
    `Sum individual non-root physical edges=${ecosystem.sumIndividualNonRootPhysicalEdges}`
  );
  report.push(
    `Sum individual root/direct edges=${ecosystem.sumIndividualRootDirectEdges}`
  );
  report.push(
    `Sum individual total physical edges including root=${ecosystem.sumIndividualTotalPhysicalEdgesIncludingRoot}`
  );
  report.push(
    `Unique exact non-root edges across ecosystem=${ecosystem.uniqueExactNonRootEdgesAcrossEcosystem}`
  );
  report.push(
    `Exact non-root edges shared by >=2 products=${ecosystem.exactEdgesSharedGE2}`
  );
  report.push(
    `Exact non-root edges shared by all ${ecosystem.K} products=${ecosystem.exactEdgesSharedAllK}`
  );
  report.push(
    `Same-name/different-version overlaps=${ecosystem.sameNameDifferentVersionOverlapCount}`
  );
  ...
  for (const n of ecosystem.sharedNodes) {
    report.push(
      `${n.node} | products=${n.products.join(",")} | count=${n.productCount} | rho=${n.rho.toFixed(3)}`
    );
  }
  ...
  for (const e of ecosystem.sharedEdges) {
    report.push(
      `${e.edge} | products=${e.products.join(",")} | count=${e.productCount} | rho=${e.rho.toFixed(3)}`
    );
  }
  ...
  report.push("--------------");
  report.push(
    "Physical topology and exact cross-product identity are intentionally separated."
  );
  report.push(
    "Topology reach rho measures structural exposure only; it is not runtime activation, conditional propagation, or natural incident probability."
  );

  fs.writeFileSync(
    path.join(OUTDIR, "ecosystem-topology-4product-corrected-report.txt"),
    report.join("\n"),
    "utf8"
  );
