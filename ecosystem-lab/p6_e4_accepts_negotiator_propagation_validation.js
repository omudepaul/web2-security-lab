...
/*
  P6 E4 ACCEPTS -> NEGOTIATOR PHASE-A PROPAGATION VALIDATION

  Exact edge: accepts@1.3.8 -> negotiator@0.6.3
  Product: socket.io@4.8.1 / engine.io@6.6.10
...
const OUTDIR = path.join(
  ROOT,
  "results",
  "e4-p6-accepts-negotiator-phase-a-propagation-validation"
);
...
function verifyVersions() {
  ...
  // Verify actual Node resolution, not merely top-level package labels.
  const polling = path.join(NODE_MODULES, "engine.io", "build", "transports", "polling.js");
  const acceptsFile = Module.createRequire(polling).resolve("accepts");
  const negotiatorFile = Module.createRequire(acceptsFile).resolve("negotiator");
  for (const [actual, expectedFile] of [
    [acceptsFile, path.join(NODE_MODULES, "accepts", "index.js")],
    [negotiatorFile, path.join(NODE_MODULES, "negotiator", "index.js")],
  ]) {
    if (fs.realpathSync(actual) !== fs.realpathSync(expectedFile)) {
      throw new Error("Unexpected dependency copy: " + actual);
    }
  }
  return { ...versions, resolvedAccepts: acceptsFile, resolvedNegotiator: negotiatorFile };
}
...
async function worker() {
  ...
  const pollingFile = path.normalize(path.join(NODE_MODULES, "engine.io", "build", "transports", "polling.js"));
  ...
  // Component-level polling doWrite harness, not a full Socket.IO session.
  const server = http.createServer((req, res) => {
    req._query = { EIO: "4", transport: "polling" };
    const transport = new Polling(req);
    transport.req = req;
    transport.res = res;
    transport.httpCompression = { threshold: THRESHOLD };
    transport.doWrite(payload, { compress: true }, error => {
      writeCallbackCount += 1;
      if (error) writeCallbackError = String(error);
    });
  });
  ...
  process.stdout.write(JSON.stringify({
    ...
    writeCallbackCount,
    writeCallbackError,
    callbackOnce: writeCallbackCount === 1 && writeCallbackError === null ? 1 : 0,
    decodeError,
    requestError,
    elapsedMs,
...
function parent() {
  ...
  for (const context of CONTEXTS) {
    for (const condition of CONDITIONS) {
      ...
      summaries.push({
        ...
        piControlled: condition === "BASELINE" ? null : (observedAlphaRate === 0 ? 0 : (q === null ? null : observedAlphaRate * q)),
        ...
        callbackOnceRate: mean(rows.map(row => row.callbackOnce)),
  ...
  const allComplete =
    ...
    summaries.every(row =>
      row.n === REPS && row.http200Rate === 1 && row.bodyIntegrityRate === 1 && row.callbackOnceRate === 1
    );
  ...
  const result = {
    experiment: "P6 E4 accepts-negotiator Phase-A deterministic propagation validation",
    harnessScope: "Engine.IO Polling.doWrite over loopback HTTP; not a full Socket.IO session",
    protocolCommit: "ab8e1d3bec69e2aa3ac837b9fae77fdddcb9f4d2",
  ...
  const trialColumns = [
    ...
    "writeCallbackCount", "writeCallbackError", "callbackOnce",
  ...
  const summaryColumns = [
    ...
    "bodyIntegrityRate", "callbackOnceRate", "encodingPredictionAccuracy",
  ...
  const report = [
    ...
    ...summaries.flatMap(summary => [
      ...
      `  HTTP 200=${fmt(summary.http200Rate)}; body integrity=${fmt(summary.bodyIntegrityRate)}; callback once=${fmt(summary.callbackOnceRate)}; encoding accuracy=${fmt(summary.encodingPredictionAccuracy)}`,
    ]),
    "",
    "DECISION",
    "--------",
    ...
    `PRE-REGISTERED P6 E4 PHASE-A PROPAGATION HYPOTHESIS: ${passed ? "PASS" : "FAIL"}`,
    "",
    ...
    "Baseline q and pi are NA: no perturbation was assigned.",
    "Harness scope: Engine.IO Polling.doWrite, not a full Socket.IO session.",
    "Forced deflate is a deliberately injected negotiation error, not normal client compatibility.",
    "",
  ].join("\n");

  fs.writeFileSync(
    path.join(OUTDIR, "e4-p6-phase-a-propagation.json"),
    JSON.stringify(result, null, 2) + "\n"
  );
  fs.writeFileSync(
    path.join(OUTDIR, "e4-p6-phase-a-propagation-trials.csv"),
    toCsv(trials, trialColumns)
  );
  fs.writeFileSync(
    path.join(OUTDIR, "e4-p6-phase-a-propagation-summary.csv"),
    toCsv(summaries, summaryColumns)
  );
  fs.writeFileSync(
    path.join(OUTDIR, "e4-p6-phase-a-propagation-report.txt"),
    report
  );
